...
These mock the Anthropic client so no API key is needed in CI.
"""
...
def _mock_response(text: str, input_tokens=50, output_tokens=80):
    msg = MagicMock()
...
def test_planner_returns_subtasks():
    ...
    with patch.object(
        planner._client.messages, "create", return_value=_mock_response(fake_reply)
    ):
    ...
    assert result["metrics"][0]["agent"] == "planner"
...
def test_planner_records_latency():
    ...
    fake_reply = json.dumps({"subtasks": ["a", "b"]})
...
def test_critic_scores_report():
    ...
    with patch.object(
        critic._client.messages, "create", return_value=_mock_response(fake_reply)
    ):
